...
export const Text: React.FC<TextProps> = ({
  ...
  return (
    <Brick
      as={RNText}
      allowFontScaling={
        UnistylesRuntime.themeName === 'juicedAllowFontScaling'
          ? allowFontScaling
          : true
      }
      style={[styles.text(context), style]}
      {...props}
    >
      {children}
    </Brick>
...
const styles = StyleSheet.create(theme => {
  ...
  const colorVariants = mapVariants(
    { ...theme.colors.text, ...theme.colors.status, ...theme.brand.colors },
    'color',
    theme.colors.text.primary,
  );
  const fontSizes = mapVariants(
    theme.fontSizes,
    'fontSize',
    theme.fontSizes.md,
  );
  const lineHeights = mapVariants(
    theme.lineHeights,
    'lineHeight',
